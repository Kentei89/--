...
def render_saju_card(name, pillars, corr_dt, corrections, gender, year,
    ...
    with st.expander("⚡ 상신(相神) — 용신을 보좌하는 오행", expanded=False):
        st.info("상신은 용신을 도와 格局을 안정시키는 보조 오행이에요. 용신이 제 역할을 하도록 옆에서 받쳐주는 존재예요.")
    ...
    with st.expander("🌡️ 조후(調候) — 계절의 균형", expanded=False):
        st.info("조후는 출생 계절의 기운 균형이에요. 계절마다 부족한 오행이 다르고, 그 기운을 보충하면 삶이 더 편안해져요.")
    ...
    with st.expander("💼 직업 적성", expanded=False):
        ...
        if _jb_yjobs:
            ...
            _yb_chips = '<div style="display:flex;flex-wrap:wrap;gap:7px;margin:6px 0 10px;">'
        ...
        _jb_gyeok_cat = _GYEOK_CAT_MAP.get(_jb_gyeok)
        _jb_gyeok_cmt = _GYEOK_YONG_JOB_COMMENT.get((_jb_gyeok_cat, _jb_yong), '') if _jb_gyeok_cat else ''
        if _jb_gyeok_cmt:
            st.markdown(
                f'<div style="background:linear-gradient(135deg,#f0fdf4,#dcfce7);'
                f'border:1px solid #86efac;border-radius:10px;padding:11px 14px;margin-bottom:8px;">'
                f'<div style="font-size:0.65rem;color:#9ca3af;font-weight:600;margin-bottom:4px;">✨ {name}님 맞춤 직업 코멘트</div>'
                f'<div style="font-size:0.82rem;color:#14532d;line-height:1.65;">{_jb_gyeok_cmt}</div>'
                f'</div>',
                unsafe_allow_html=True,
            )
    ...
    if _gm_hits:
        ...
        with st.expander("🕳 공망(空亡) — 기둥별 상세 해설", expanded=False):
            st.info("공망은 해당 기둥의 기운이 허한 상태예요. 조상·부모·나·자녀 중 어느 자리에 공망이 있는지에 따라 그 분야가 약해질 수 있어요.")
    ...
    if _hy_list:
        with st.expander("⚡ 지지형(地支刑) 상세 해설", expanded=False):
            st.info("지지형은 지지끼리 서로 충돌하는 관계예요. 형이 생기면 해당 분야에서 긴장·마찰·사건이 발생할 수 있어요.")
    ...
    with st.expander("🌿 지장간(支藏干) — 지지 속 숨은 천간", expanded=False):
        st.info("지장간은 12지지 안에 숨어있는 천간이에요. 표면에 드러나지 않는 숨은 에너지로, 운이 발동할 때 튀어나와 영향을 줘요.")
    ...
    with st.expander("🌙 월운(月運) — 이달·다음달 운세", expanded=False):
        st.info("월운은 매월 바뀌는 이달의 운이에요. 세운 안에서 이달·다음달 좋은 시기와 주의할 시기를 월별로 확인할 수 있어요.")
    ...
    with st.expander("🌊 대운(大運) — 10년 주기 큰 흐름", expanded=False):
        st.info("대운은 약 10년 단위로 바뀌는 큰 운의 흐름이에요. 지금 어떤 대운에 있느냐에 따라 삶의 방향과 기회가 달라져요.")
    ...
    with st.expander("📅 세운(歲運) — 연도별 운세 표", expanded=False):
        st.info("세운은 매년 바뀌는 그해의 운이에요. 대운의 큰 흐름 안에서 해마다 어떤 변화와 기회가 오는지 구체적으로 보여줘요.")
    ...
    with st.expander("🔖 신살(神殺) — 귀인·흉살 상세 해설", expanded=False):
        st.info("신살은 사주에 나타나는 특별한 별이에요. 귀인성은 도움을 주는 별, 흉살은 주의가 필요한 별이에요. 타고난 인연과 조심할 시기를 알려줘요.")
...
# 60甲子 연애·관계 패턴
_ILJU_RELATION = {
    (0,0):  '이성에게 진지하고 신중하게 다가가지만 상대가 먼저 마음을 열기를 기다리는 편이에요. 지식과 가치관이 맞는 파트너와 깊이 있는 관계를 만들어가요.',
    (0,2):  '솔직하고 당당한 매력으로 상대를 끌어당기지만 관계에서도 독립성을 중요하게 여겨요. 나의 공간을 존중해주는 파트너와 가장 잘 맞아요.',
    (0,4):  '겉은 든든하고 믿음직하지만 내면에는 깊은 감성과 이상이 있어요. 안정감을 주면서도 함께 성장해줄 수 있는 파트너에게 마음을 열어요.',
    (0,6):  '열정적이고 솔직한 표현으로 관계를 이끄는 편이에요. 같은 열정과 에너지를 가진 파트너와 함께할 때 시너지가 폭발해요.',
    (0,8):  '냉철하고 이성적인 외모 뒤에 깊은 감수성이 숨어 있어요. 지적이고 대화가 통하는 파트너와 점차 신뢰를 쌓아가는 방식을 선호해요.',
    (0,10): '원칙을 중요하게 여기는 성향이 강해 처음에는 거리감이 느껴질 수 있지만, 신뢰가 쌓이면 깊고 진한 관계를 유지해요. 가치관이 일치하는 사람과 오래가요.',
    (1,1):  '겉으로는 조용하고 차분하지만 관계에서 강한 내면의 고집이 있어요. 자신의 리듬을 존중해주는 파트너와 오랜 시간 신뢰를 쌓아가요.',
    (1,3):  '감성이 풍부하고 공감 능력이 뛰어나 자연스럽게 사람을 끌어당겨요. 창의적이고 감성적인 파트너와 특히 교류가 잘 맞아요.',
    (1,5):  '신비로운 매력과 전략적인 면이 공존해 처음에는 파악하기 어려운 인상을 줘요. 깊이 있고 지적인 파트너와 단계적으로 신뢰를 쌓는 방식을 선호해요.',
    (1,7):  '따뜻하고 배려 깊은 성향으로 자연스럽게 사람들의 마음을 끌어요. 정서적으로 안정된 파트너와 함께할 때 가장 편안하고 행복해요.',
    (1,9):  '완벽주의적 성향이 관계에도 적용되어 기준이 높은 편이에요. 섬세함과 세련됨을 이해해줄 수 있는 파트너와 깊은 관계를 맺어요.',
    (1,11): '이상주의적이고 낭만적인 성향으로 깊고 의미 있는 관계를 추구해요. 감성과 지성을 함께 갖춘 파트너와 철학적인 교류를 즐겨요.',
    (2,0):  '뜨거운 외면 뒤에 의외로 깊고 내성적인 감수성이 있어요. 진지하고 신뢰할 수 있는 파트너와 안정적인 관계를 이루면 오래가요.',
    (2,2):  '열정적이고 활기 넘치는 에너지로 자연스럽게 사람들의 중심이 돼요. 비슷한 에너지와 목표를 가진 파트너와 함께 앞으로 전진하는 관계를 원해요.',
    (2,4):  '다재다능하고 포용적인 성향으로 다양한 사람들과 잘 어울려요. 안정감과 개방성을 동시에 갖춘 파트너와 함께 풍요로운 관계를 만들어요.',
    (2,6):  '강렬한 에너지와 솔직함이 관계에서도 그대로 드러나요. 뜨거운 열정을 받아줄 수 있는 강한 파트너가 필요해요. 감정 기복을 조절하는 것이 좋은 관계의 열쇠예요.',
    (2,8):  '냉철함과 분석적인 면이 관계에서도 드러나 처음에는 거리감이 있어 보여요. 지적이고 실용적인 파트너와 점차 신뢰를 쌓아 깊은 관계로 발전해요.',
    (2,10): '원칙과 고집이 관계에서도 드러나 처음에는 까다롭게 느껴질 수 있어요. 자신의 기준에 맞는 파트너와 깊은 신뢰를 쌓으면 매우 헌신적인 관계를 만들어요.',
    (3,1):  '섬세하고 신중한 성향으로 쉽게 마음을 열지 않지만, 한번 신뢰하면 매우 헌신적이에요. 현실적이고 꾸준한 파트너와 안정적인 관계를 만들어가요.',
    (3,3):  '따뜻한 감수성과 창의성으로 자연스럽게 사람들의 마음을 열어요. 예술적·감성적 감각을 공유할 수 있는 파트너와 깊은 교류를 즐겨요.',
    (3,5):  '집중력과 욕망이 관계에서도 드러나 원하는 파트너에게 강하게 집착하는 경향이 있어요. 상대방의 독립성을 존중하는 법을 배우면 더 건강한 관계가 돼요.',
    (3,7):  '따뜻한 감성 지능으로 사람들의 마음을 자연스럽게 이해하고 배려해요. 정서적으로 교감할 수 있는 파트너와 진심 어린 관계를 만들어요.',
    (3,9):  '완벽주의적이고 세련된 기준이 관계에서도 높아 쉽게 만족하지 않아요. 자신만의 기준과 감각을 공유할 수 있는 파트너와 깊은 관계를 맺어요.',
    (3,11): '지혜롭고 낭만적인 성향으로 깊은 이야기를 나눌 수 있는 파트너를 원해요. 정신적 교감이 강한 관계에서 행복을 느껴요.',
    (4,0):  '든든한 외면과 달리 내면에는 지적 감수성이 있어요. 안정감을 주면서도 지적 교류가 가능한 파트너와 함께 균형 잡힌 관계를 만들어요.',
    (4,2):  '대범하고 포용적이어서 다양한 사람들과 잘 어울려요. 리더십 있고 활동적인 파트너와 함께 큰 목표를 향해 나아가는 관계를 원해요.',
    (4,4):  '묵직하고 고집스러운 성향이 관계에서도 드러나요. 안정적이고 신뢰할 수 있는 파트너와 장기적인 관계를 선호하며, 변화보다는 지속성을 중시해요.',
    (4,6):  '열정적이고 직선적인 성향으로 관계에서도 솔직하게 감정을 표현해요. 비슷한 에너지와 목표를 공유하는 파트너와 활발한 관계를 만들어가요.',
    (4,8):  '실용적이고 분석적인 성향으로 관계를 현실적으로 바라봐요. 실용적이고 합리적인 파트너와 함께 구체적인 계획을 세우는 관계가 잘 맞아요.',
    (4,10): '원칙이 강하고 완고한 성향이 관계에서도 드러나요. 자신의 가치관을 존중해주는 파트너와 신뢰를 쌓으면 매우 헌신적이고 든든한 관계가 돼요.',
    (5,1):  '성실하고 현실적인 성향으로 관계에서도 믿음직한 모습을 보여요. 안정적이고 꾸준한 파트너와 오랜 시간 신뢰를 쌓아가는 관계가 잘 맞아요.',
    (5,3):  '따뜻하고 배려 깊은 성향으로 관계에서 감성적인 연결을 중요하게 여겨요. 창의적이고 감성이 풍부한 파트너와 교류가 풍요로워요.',
    (5,5):  '집중력이 강하고 전략적인 성향이 관계에서도 목적의식으로 나타나요. 비슷한 목표 의식과 실행력을 가진 파트너와 함께 성장하는 관계를 원해요.',
    (5,7):  '섬세하고 온화한 성향으로 사람들과 자연스럽게 어울려요. 포용력 있고 안정적인 파트너와 함께 따뜻한 관계를 오랫동안 유지해요.',
    (5,9):  '자존감이 강하고 정밀한 성향이 관계에서도 높은 기준으로 나타나요. 세련되고 전문성 있는 파트너와 깊은 교류를 나눠요.',
    (5,11): '이상주의적이고 지적인 성향으로 깊이 있는 대화와 철학적 교류를 즐겨요. 학문적이고 사려 깊은 파트너와 오랜 시간 함께 성장해요.',
    (6,0):  '냉철하고 전략적인 외면 뒤에 날카로운 통찰력이 있어요. 지적이고 실력 있는 파트너와 공통의 목표를 향해 함께 나아가는 관계를 원해요.',
    (6,2):  '카리스마와 직선적인 에너지로 관계에서도 강하게 존재감을 드러내요. 자신의 강한 성향을 감당할 수 있는 파트너와 함께해야 균형이 맞아요.',
    (6,4):  '포용력 있고 든든한 성향이 관계에서도 신뢰를 만들어요. 안정적이고 실력 있는 파트너와 함께 탄탄한 기반을 쌓아가는 관계를 원해요.',
    (6,6):  '강렬한 충동과 열정이 관계에서도 뜨겁게 드러나요. 역동적인 파트너와 함께하면 시너지가 나지만, 지속성을 위해 안정감도 필요해요.',
    (6,8):  '완벽주의적이고 독립적인 성향이 강해 관계에서도 자신의 기준을 잘 굽히지 않아요. 독립성을 존중해주는 파트너와 함께할 때 가장 잘 맞아요.',
    (6,10): '원칙주의적이고 고독한 성향이 관계에서도 드러나 쉽게 마음을 열지 않아요. 자신의 세계를 이해하고 존중해주는 파트너와 깊은 신뢰 관계를 형성해요.',
    (7,1):  '섬세하고 냉철한 성향이 관계에서도 쉽게 마음을 열지 않는 것으로 나타나요. 인내심 있고 안정적인 파트너와 천천히 신뢰를 쌓아가면 깊은 관계가 돼요.',
    (7,3):  '예술적 감수성과 유연한 적응력으로 다양한 사람들과 자연스럽게 교류해요. 창의적이고 감성이 풍부한 파트너와 특히 깊은 공감대가 형성돼요.',
    (7,5):  '전략적이고 날카로운 성향이 관계에서도 냉철한 판단으로 나타나요. 실력 있고 신뢰할 수 있는 파트너와 합리적인 관계를 구축해요.',
    (7,7):  '온화하고 감성적인 성향으로 주변 사람들을 자연스럽게 배려해요. 따뜻하고 사려 깊은 파트너와 안정적이고 행복한 관계를 이뤄요.',
    (7,9):  '완벽주의와 높은 자존심이 관계에서도 타협하지 않는 기준으로 나타나요. 자신의 가치를 인정해주는 파트너와 함께할 때 가장 만족스러운 관계가 돼요.',
    (7,11): '낭만적이고 지적인 성향으로 깊이 있는 대화와 철학적 교류를 즐겨요. 지성과 감성을 갖춘 파트너와 예술적·학문적 교류가 풍요로운 관계를 원해요.',
    (8,0):  '지혜롭고 통찰력 있는 성향으로 관계에서도 깊이 있는 교류를 원해요. 지적이고 감성적인 파트너와 철학적인 대화를 나누는 관계가 잘 맞아요.',
    (8,2):  '활동적이고 다재다능한 성향으로 다양한 사람들과 자연스럽게 어울려요. 비슷한 에너지와 호기심을 가진 파트너와 함께 새로운 경험을 탐색하는 관계를 원해요.',
    (8,4):  '포용력이 크고 다양성을 품는 성향으로 다양한 사람들을 자연스럽게 받아들여요. 깊이 있고 안정적인 파트너와 장기적이고 견고한 관계를 만들어요.',
    (8,6):  '강렬하고 역동적인 에너지가 관계에서도 활발하게 드러나요. 빠른 판단과 결정을 즐기지만 관계의 안정성을 위해 감정 기복을 조율하는 것이 중요해요.',
    (8,8):  '지혜롭고 분석적인 성향으로 관계를 전략적으로 접근하는 경향이 있어요. 지적이고 실용적인 파트너와 함께 목표를 공유하며 발전하는 관계를 원해요.',
    (8,10): '원칙적이고 깊이 있는 성향이 관계에서도 신뢰와 무게감으로 나타나요. 자신의 원칙을 이해하고 존중해주는 파트너와 오래가는 관계를 형성해요.',
    (9,1):  '꼼꼼하고 인내심 있는 성향으로 관계에서도 신뢰를 가장 중요하게 여겨요. 안정적이고 현실적인 파트너와 함께 오래가는 관계를 만들어가요.',
    (9,3):  '감성적이고 창의적인 성향으로 다양한 사람들과 자연스럽게 공감대를 형성해요. 예술적이고 감성이 풍부한 파트너와 깊은 교류를 즐겨요.',
    (9,5):  '집중력이 강하고 신비로운 매력이 있어 처음에는 파악하기 어렵지만, 깊이 사귀면 강한 유대감이 형성돼요. 지적이고 목표 의식 있는 파트너와 함께 성장하는 관계를 원해요.',
    (9,7):  '높은 감성 지능과 따뜻한 배려로 자연스럽게 사람들의 마음을 이끌어요. 감성적이고 사려 깊은 파트너와 서로를 잘 이해하는 관계를 만들어요.',
    (9,9):  '섬세하고 완벽주의적인 성향이 관계에서도 높은 기준과 정밀함으로 나타나요. 전문성과 세련됨을 갖춘 파트너와 깊은 교류를 나눠요.',
    (9,11): '깊은 이상주의와 내면 지향적 성향으로 관계에서도 철학적이고 깊이 있는 교류를 원해요. 지혜롭고 내면이 깊은 파트너와 정신적으로 연결되는 관계가 가장 행복해요.',
}

# 60甲子 삶의 방향·조언
_ILJU_HINT = {
    (0,0):  '지적 탐구와 행동력이 결합된 이 일주는 새로운 분야를 개척하는 데 강점이 있어요. 한 가지에 집중하면 누구도 따라오기 어려운 전문가가 될 수 있어요.',
    (0,2):  '개척정신과 리더십이 강점인 이 일주는 자신만의 길을 걸어가는 선구자 스타일이에요. 남의 시선보다 자신의 신념을 따를 때 가장 빛나요.',
    (0,4):  '전문성과 포용력이 결합된 이 일주는 탄탄한 기반 위에서 큰 성과를 만들어요. 서두르지 않고 꾸준히 쌓아갈 때 장기적으로 강한 힘이 발휘돼요.',
    (0,6):  '표현력과 열정이 강점인 이 일주는 많은 사람들에게 영감을 줄 수 있어요. 에너지를 지속 가능하게 관리하면서 사람과 연결될 때 가장 큰 임팩트를 만들어요.',
    (0,8):  '날카로운 분석력과 강한 의지가 결합된 이 일주는 어려운 환경에서 오히려 진가를 발휘해요. 경쟁이 있는 분야에서 도전할수록 더 강해지는 스타일이에요.',
    (0,10): '고집과 원칙이 결합된 이 일주는 한 분야를 오래 파고들었을 때 독보적인 전문가가 돼요. 유연함을 조금 더 수용하면 훨씬 많은 문이 열려요.',
    (1,1):  '어떤 환경에서도 살아남는 강한 생명력이 이 일주의 최대 강점이에요. 인내와 꾸준함으로 서서히 쌓아가면 남들이 놀랄 만한 성과를 만들어내요.',
    (1,3):  '창의성과 감수성이 강점인 이 일주는 예술·교육·콘텐츠처럼 감성이 가치가 되는 분야에서 특히 빛나요. 자신의 감성을 신뢰하는 것이 가장 중요해요.',
    (1,5):  '집중력과 전략적 사고가 결합된 이 일주는 목표를 향해 조용하지만 강하게 전진하는 스타일이에요. 드러내지 않고 쌓아가는 힘이 결국 큰 성과로 이어져요.',
    (1,7):  '따뜻한 감성과 섬세한 배려가 강점인 이 일주는 사람을 돌보고 연결하는 역할에서 탁월해요. 자신도 충분히 보살핌을 받는 것이 중요해요.',
    (1,9):  '정밀함과 완성도 추구가 이 일주의 핵심 강점이에요. 기술·예술·금융처럼 전문성이 요구되는 분야에서 기준을 높게 유지할수록 더 높은 가치를 인정받아요.',
    (1,11): '지적 감성과 이상주의가 결합된 이 일주는 의미와 깊이를 추구하는 분야에서 독보적인 위치를 만들어요. 이상을 현실과 연결하는 능력을 키우면 빛을 발해요.',
    (2,0):  '열정과 지혜가 결합된 이 일주는 사람들을 이끌면서도 깊이 있게 사고하는 능력이 있어요. 표면의 에너지와 내면의 통찰을 함께 활용하면 남다른 성과를 만들어요.',
    (2,2):  '리더십과 추진력이 넘치는 이 일주는 가만히 있는 것보다 도전하고 실행할 때 가장 강해요. 자신의 에너지를 지속 가능한 방향으로 관리하는 것이 장기 성공의 열쇠예요.',
    (2,4):  '다방면의 재능과 포용력이 강점인 이 일주는 여러 분야를 연결하는 역할에서 특히 뛰어나요. 다양성을 강점으로 활용할수록 더 많은 기회가 열려요.',
    (2,6):  '강렬한 열정과 추진력이 이 일주의 최대 강점이에요. 소진되지 않도록 에너지를 전략적으로 분배하는 것이 장기적인 성공에 중요해요.',
    (2,8):  '분석력과 변화 적응력이 결합된 이 일주는 빠르게 변화하는 환경에서 기회를 찾는 능력이 탁월해요. IT·금융·분석직에서 강한 경쟁력을 발휘해요.',
    (2,10): '전문성과 원칙에 기반한 이 일주는 한 분야를 오래 파고들수록 빛을 발해요. 법률·종교·연구처럼 깊이가 필요한 분야에서 탁월한 권위자가 될 수 있어요.',
    (3,1):  '인내와 섬세함이 결합된 이 일주는 꾸준히 쌓아가는 힘이 강해요. 빠른 성과를 쫓기보다 한 걸음씩 확실하게 나아갈 때 가장 견고한 결과가 나와요.',
    (3,3):  '감수성과 창의성이 강점인 이 일주는 예술·교육·상담처럼 사람의 마음을 움직이는 분야에서 탁월한 능력을 발휘해요. 자신의 감성을 충분히 표현할 환경이 중요해요.',
    (3,5):  '강한 집중력과 비즈니스 감각이 결합된 이 일주는 목표를 향해 전력질주하는 능력이 있어요. 에너지를 지속적으로 관리하면서 분산하지 않을 때 최고의 성과가 나와요.',
    (3,7):  '감성 지능과 예술성이 강점인 이 일주는 사람을 연결하고 돌보는 역할에서 빛나요. 자신의 따뜻함을 지치지 않게 유지하는 것이 삶의 중요한 과제예요.',
    (3,9):  '정밀함과 예술적 완성도가 강점인 이 일주는 기술·예술·의료처럼 완성도가 가치인 분야에서 최고를 추구해요. 자신의 기준을 유지하면서도 유연성을 키우는 것이 중요해요.',
    (3,11): '지혜와 이상주의가 결합된 이 일주는 깊이 있는 전문성을 쌓는 분야에서 오래 빛나요. 현실과 이상의 균형을 찾는 것이 삶의 핵심 과제예요.',
    (4,0):  '포용력과 지적 통찰이 결합된 이 일주는 다양한 사람들을 품으면서도 전략적으로 사고하는 능력이 있어요. 안정과 혁신의 균형을 잡는 것이 삶의 핵심이에요.',
    (4,2):  '대범한 리더십과 행동력이 강점인 이 일주는 큰 그림을 그리고 실행하는 역할에서 뛰어나요. 세밀한 부분도 놓치지 않는 습관을 기르면 완성도가 더욱 높아져요.',
    (4,4):  '강한 저력과 인내가 이 일주의 핵심 강점이에요. 부동산·건설·토지처럼 실물 기반의 분야에서 장기 보유 전략을 유지하면 큰 자산을 만들어요.',
    (4,6):  '리더십과 열정이 결합된 이 일주는 에너지 기반의 활동적인 분야에서 강한 성과를 만들어요. 씀씀이가 클 수 있으니 수입과 지출의 균형을 잡는 것이 중요해요.',
    (4,8):  '실용성과 변화 적응력이 강점인 이 일주는 빠른 환경 변화 속에서 기회를 포착하는 능력이 탁월해요. IT·물류·금융 분야에서 행동력을 발휘하면 성과가 커요.',
    (4,10): '원칙과 인내가 결합된 이 일주는 오랜 시간 쌓아온 것이 가장 강한 자산이에요. 법조·연구·부동산처럼 깊이와 신뢰가 필요한 분야에서 탁월해요.',
    (5,1):  '성실함과 현실 감각이 강점인 이 일주는 꾸준히 관리하고 쌓아가는 능력이 탁월해요. 재물 관리와 부동산 분야에서 특히 강한 경쟁력이 나와요.',
    (5,3):  '감성과 유연성이 결합된 이 일주는 사람과 사람을 연결하고 돌보는 역할에서 빛나요. 서비스·교육·예술 분야에서 관계 기반의 수입이 자연스럽게 만들어져요.',
    (5,5):  '목표 지향적인 집중력과 전략적 사고가 이 일주의 핵심 강점이에요. 금융·사업·전략 기획 분야에서 명확한 목표를 향해 집중할수록 큰 성과가 나와요.',
    (5,7):  '온화함과 섬세함이 결합된 이 일주는 음식·복지·상담처럼 사람의 삶에 밀착되는 분야에서 꾸준히 빛나요. 안정적인 기반을 먼저 확보하고 성장하는 방식이 맞아요.',
    (5,9):  '정밀함과 자존감이 강점인 이 일주는 기술·품질 기반 분야에서 완성도 높은 결과물로 프리미엄 수익을 만들어요. 기준을 유지하면서 실행력을 더하면 최고가 돼요.',
    (5,11): '지혜와 학문적 깊이가 강점인 이 일주는 전문 지식을 활용하는 분야에서 독보적인 위치를 만들어요. 지식을 현실로 연결하는 실행력을 키우면 훨씬 더 빛나요.',
    (6,0):  '지혜와 실행력이 결합된 이 일주는 금융·IT·전략 분야에서 지식과 행동의 시너지로 큰 성과를 만들어요. 분석하고 실행하는 사이클을 빠르게 돌릴수록 앞서가요.',
    (6,2):  '강렬한 추진력과 도전 정신이 이 일주의 최대 강점이에요. 사업·무역·스포츠처럼 경쟁이 있는 분야에서 거침없이 도전할수록 더 강해져요.',
    (6,4):  '저력과 전문성이 결합된 이 일주는 탄탄한 기반 위에서 꾸준히 쌓아가는 방식이 맞아요. 부동산·기술·법조 분야에서 오래 파고들수록 강한 자산이 만들어져요.',
    (6,6):  '폭발적인 에너지와 추진력이 강점이지만 충동적인 결정을 조심해야 해요. 에너지를 전략적으로 관리하고 장기적인 시각을 유지할 때 최고의 성과가 나와요.',
    (6,8):  '냉철한 완벽주의가 이 일주의 핵심 강점이에요. 기술·의료·금융 분야에서 높은 기준을 유지하면 고수익과 명성이 자연스럽게 따라와요.',
    (6,10): '깊은 전문성과 원칙이 결합된 이 일주는 한 분야를 오래 파고들수록 독보적인 권위자가 돼요. 유연성을 조금 더 허용하면 더 많은 기회가 찾아와요.',
    (7,1):  '인내와 섬세함이 결합된 이 일주는 보석·미용·정밀 기술처럼 섬세함이 가치가 되는 분야에서 탁월해요. 꾸준히 쌓아가는 과정을 즐길 때 가장 빛나요.',
    (7,3):  '예술성과 감각이 결합된 이 일주는 패션·콘텐츠·예술처럼 감각이 돈이 되는 분야에서 특히 강해요. 자신의 미적 감각을 믿고 표현하면 독보적인 브랜드가 돼요.',
    (7,5):  '전략적 사고와 날카로운 판단력이 이 일주의 핵심 강점이에요. 금융·법률·전략 분야에서 조용하지만 강한 실행력으로 큰 성과를 만들어요.',
    (7,7):  '온화함과 배려가 강점인 이 일주는 서비스·예술·상담처럼 사람의 마음을 다루는 분야에서 탁월해요. 자신의 따뜻한 에너지를 지속 가능하게 유지하는 것이 중요해요.',
    (7,9):  '정밀함과 완벽주의가 이 일주의 최대 강점이에요. 기술·의료·보석처럼 완성도가 최고 가치인 분야에서 타협 없는 기준을 유지하면 독보적인 전문가가 돼요.',
    (7,11): '지혜와 예술성이 결합된 이 일주는 학문·예술·상담처럼 전문성과 감성이 함께 필요한 분야에서 독보적인 위치를 만들어요. 내면의 예리함을 믿고 표현하세요.',
    (8,0):  '지혜와 통찰이 극대화된 이 일주는 정보·금융·전략 분야에서 흐름을 읽는 능력이 최고예요. 내면의 통찰을 실행으로 연결하는 것이 가장 중요한 과제예요.',
    (8,2):  '활동력과 다재다능함이 강점인 이 일주는 무역·사업·IT처럼 활발하게 움직일수록 수입이 늘어나는 분야가 잘 맞아요. 다양한 경험이 최고의 자산이에요.',
    (8,4):  '포용력과 저력이 강점인 이 일주는 부동산·금융·법조처럼 내부에 쌓인 자원이 가치가 되는 분야에서 강해요. 넓게 품으면서도 중심을 잃지 않는 것이 핵심이에요.',
    (8,6):  '역동적인 에너지와 행동력이 이 일주의 핵심 강점이에요. 재물 흐름이 빠른 분야에서 활발하게 움직이되, 수입과 지출의 균형을 잡는 습관이 필요해요.',
    (8,8):  '지혜와 변화 적응력이 결합된 이 일주는 IT·금융·전략처럼 변화가 빠른 분야에서 기회를 빠르게 포착해요. 분석하고 실행하는 싸이클을 반복할수록 앞서가요.',
    (8,10): '원칙과 깊이가 결합된 이 일주는 장기적으로 전문성을 쌓아가는 방식이 맞아요. 법조·연구·전문직에서 오래 쌓은 것이 노년까지 강한 자산이 돼요.',
    (9,1):  '인내와 현실 감각이 강점인 이 일주는 꾸준한 관리와 저축으로 안정적인 자산을 쌓아요. 부동산·금융처럼 장기 보유가 강점인 분야가 잘 맞아요.',
    (9,3):  '감성과 창의성이 강점인 이 일주는 예술·콘텐츠·교육처럼 감성이 가치가 되는 분야에서 빛을 발해요. 자신의 감성을 출구로 표현할수록 행복과 성과가 함께 와요.',
    (9,5):  '집중력과 신비로운 전략 사고가 이 일주의 강점이에요. 금융·기획·전략 분야에서 드러내지 않고 조용히 쌓아가는 방식으로 최고의 성과를 만들어요.',
    (9,7):  '감성 지능과 온화함이 강점인 이 일주는 상담·서비스·복지처럼 사람의 마음을 다루는 분야에서 탁월해요. 자신의 에너지를 소진하지 않도록 경계를 세우는 것이 중요해요.',
    (9,9):  '섬세함과 완벽주의가 결합된 이 일주는 정밀 기술·금융·의료처럼 완성도가 최고 가치인 분야에서 강해요. 완성도 높은 결과물이 최고의 경쟁력이에요.',
    (9,11): '깊은 지혜와 이상주의가 결합된 이 일주는 학문·철학·상담·예술처럼 깊이가 있는 분야에서 독보적인 전문가가 돼요. 내면의 세계를 충분히 탐구하고 표현하는 삶을 사세요.',
}
...
def _render_ilju_card(name, pillars, no_time=False):
    ...
    _rel_txt  = _ILJU_RELATION.get((ilgan, ilji), '')
    _hint_txt = _ILJU_HINT.get((ilgan, ilji), '')
    if _rel_txt or _hint_txt:
        _r_inner = ''
        if _rel_txt:
            _r_inner += (
                f'<div style="font-size:0.65rem;color:#9ca3af;font-weight:600;margin-bottom:4px;">💑 연애·관계 패턴</div>'
                f'<div style="font-size:0.82rem;color:#831843;line-height:1.65;margin-bottom:{"8px" if _hint_txt else "0"};">{_rel_txt}</div>'
            )
        if _hint_txt:
            _sep2 = '<div style="border-top:1px solid #fbcfe8;margin:0 0 8px;"></div>' if _rel_txt else ''
            _r_inner += (
                f'{_sep2}'
                f'<div style="font-size:0.65rem;color:#9ca3af;font-weight:600;margin-bottom:4px;">🌟 삶의 방향 조언</div>'
                f'<div style="font-size:0.82rem;color:#831843;line-height:1.65;">{_hint_txt}</div>'
            )
        st.markdown(
            f'<div style="background:linear-gradient(135deg,#fdf2f8,#fce7f3);'
            f'border:1px solid #fbcfe8;border-radius:10px;padding:11px 14px;margin-top:10px;">'
            f'{_r_inner}</div>',
            unsafe_allow_html=True,
        )
    # ── 십이운성(十二運星) ──────────────────────────────────────────
...
# 格局 카테고리 매핑
_GYEOK_CAT_MAP = {
    '건록격':'비겁', '월겁격':'비겁', '종강격':'비겁',
    '식신격':'식상', '상관격':'식상', '종아격':'식상', '염상격(炎上格)':'식상',
    '편재격':'재성', '정재격':'재성', '종재격':'재성', '가색격(稼穡格)':'재성',
    '편관격':'관성', '정관격':'관성', '종살격':'관성', '종혁격(從革格)':'관성',
    '편인격':'인성', '정인격':'인성', '곡직격(曲直格)':'인성', '윤하격(潤下格)':'인성',
}

# 格局 카테고리 × 용신오행 직업 맞춤 코멘트 (5×5=25)
_GYEOK_YONG_JOB_COMMENT = {
    ('비겁','목'): '독립심이 강한 당신이 木 기운을 만나면 자기계발과 성장이 맞물려 시너지가 나요. 교육·기획·언론처럼 지식을 성장으로 연결하는 분야에서 경쟁력이 빛을 발해요.',
    ('비겁','화'): '추진력 강한 당신에게 火 기운은 표현과 소통의 날개예요. IT·마케팅·방송처럼 에너지를 드러내는 무대에서 존재감이 특히 강해요.',
    ('비겁','토'): '자기 주도적 성향에 土 기운이 더해지면 안정적인 기반 위에서 독립을 이뤄요. 건설·유통·부동산처럼 실물 경제 분야에서 탄탄한 입지가 생겨요.',
    ('비겁','금'): '강한 개성과 金 기운이 결합되면 규율과 원칙으로 성과를 만드는 구조예요. 금융·법률·경찰 계열에서 당신의 강함이 올바른 방향을 찾아요.',
    ('비겁','수'): '독립적인 성향과 水 기운이 어우러지면 전략과 통찰이 강점이 돼요. 연구·무역·철학처럼 깊은 사고가 필요한 분야에서 경쟁력이 생겨요.',
    ('식상','목'): '창의적인 당신이 木 기운을 용신으로 삼으면 재능이 지식으로 체계화돼요. 교육·강의·출판처럼 재능을 나누는 활동에서 보람과 성과가 동시에 와요.',
    ('식상','화'): '표현력이 뛰어난 당신에게 火 기운은 무대와 관중이에요. 방송·연예·마케팅처럼 사람들 앞에 서는 일에서 재능이 빛을 발해요.',
    ('식상','토'): '풍부한 재능에 土 안정감이 더해지면 현실적인 결실이 생겨요. 요식업·식품·콘텐츠처럼 사람의 일상에 밀착된 분야에서 꾸준한 성과를 만들어요.',
    ('식상','금'): '창의성과 金 기운이 결합되면 정밀하고 전문적인 작업에서 강해요. 디자인·공예·정밀기술처럼 섬세함과 기술이 맞닿은 분야가 잘 맞아요.',
    ('식상','수'): '재능과 水 기운이 만나면 깊이 있는 연구와 창작으로 이어져요. 철학·문학·R&D처럼 사색과 표현이 함께 필요한 분야에서 독보적인 영역을 만들어요.',
    ('재성','목'): '사업 감각과 木 기운이 어우러지면 교육·환경·기획 분야에서 수익 모델을 잘 만들어요. 사람들이 성장하도록 돕는 사업 역할에서 진가를 발휘해요.',
    ('재성','화'): '재물 감각에 火 에너지가 더해지면 마케팅·IT·엔터테인먼트에서 비즈니스 성과가 나요. 트렌드를 빠르게 읽고 수익으로 연결하는 능력이 탁월해요.',
    ('재성','토'): '재성格이 土 기운을 용신으로 삼으면 부동산·건설·물류처럼 실물 자산 분야에서 특히 강해요. 꾸준한 투자와 관리가 장기적인 안정 자산을 만들어줘요.',
    ('재성','금'): '재물 지향과 金 기운이 결합되면 금융·투자·법률 분야에서 날카로운 판단력을 발휘해요. 수익과 리스크를 정밀하게 계산하는 능력이 빛을 발해요.',
    ('재성','수'): '사업적 직관과 水 기운이 만나면 무역·유통·글로벌 비즈니스에서 두각을 나타내요. 국제적인 네트워크와 전략적 사고가 시너지를 만들어요.',
    ('관성','목'): '원칙과 木 기운이 결합되면 교육·의학·상담처럼 사람을 성장시키는 권위 있는 역할에서 탁월해요. 지식과 규율을 통해 신뢰받는 리더가 되는 길이 잘 맞아요.',
    ('관성','화'): '체계와 火 기운이 어우러지면 방송·정치·강연처럼 권위 있는 무대에서 빛을 발해요. 존재감과 원칙이 함께 드러나는 환경에서 영향력이 커져요.',
    ('관성','토'): '관성格에 土 기운이 더해지면 공무원·행정·부동산처럼 안정적 체계 속에서 실력을 발휘해요. 기관·조직에서 신뢰받는 위치가 잘 맞아요.',
    ('관성','금'): '권력 지향과 金 기운의 결합은 군인·법조인·검찰처럼 강력한 권위를 다루는 분야에서 최강의 조합이에요. 규율과 실행력이 빛나는 자리가 맞아요.',
    ('관성','수'): '원칙과 水 기운이 결합되면 외교·연구·국제법처럼 지식과 권위가 맞닿는 분야에서 강해요. 깊은 전략 사고가 체계와 만나 독보적인 경쟁력이 생겨요.',
    ('인성','목'): '학문적 성향과 木 기운이 만나면 교육·의학·출판처럼 지식을 나누는 분야에서 두각을 나타내요. 사람을 성장시키는 일에서 깊은 보람과 성과를 동시에 얻어요.',
    ('인성','화'): '지식 지향에 Fire 기운이 더해지면 강연·방송·컨설팅처럼 아는 것을 세상에 드러내는 일에서 빛을 발해요. 전문가로서의 영향력이 자연스럽게 커져요.',
    ('인성','토'): '인성格이 土 기운을 용신으로 삼으면 행정·부동산·농업처럼 지식을 실용적으로 구현하는 분야에서 안정적인 성과를 만들어요.',
    ('인성','금'): '전문 지향과 金 기운이 결합되면 법조·의료·금융처럼 고도의 전문성이 필요한 분야에서 정밀함이 빛나요. 자격증·전문직 경로가 잘 맞아요.',
    ('인성','수'): '학문과 水 기운의 결합은 연구·철학·무역처럼 깊이 있는 분야에서 독보적인 전문가가 되게 해줘요. 끊임없는 탐구가 당신의 가장 큰 경쟁력이에요.',
}
